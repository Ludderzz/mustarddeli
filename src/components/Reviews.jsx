      ...
      Kind <span className="text-deli-mustard italic font-bold tracking-tight">Words</span>
        ...
        >
          {reviewsData.map((review) => (
            <div 
              key={review.id} 
              className="min-w-[85vw] md:min-w-[450px] snap-center bg-deli-grey/40 p-10 rounded-[3rem] flex flex-col justify-between border border-transparent hover:border-deli-mustard/20 transition-all duration-500"
            >
              <div>
                <Quote className="text-deli-mustard mb-6 opacity-30" size={32} />
                {/* Text changed to text-deli-mustard */}
              ...
              <div className="flex items-center justify-between mt-auto pt-6 border-t border-slate-200/50">
                ...
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest bg-white px-3 py-1 rounded-full shadow-sm">
                  {review.tag}
                </span>
              </div>
            </div>
